...
class Charger:
    ...
    def __init__(self):
        ...
        self.available_current = int(
            float(state.get("number.28_oriel_road_available_current")))
    ...
    def set_current_limit(self, current: float):
        zaptec.limit_current(blocking=False, return_response=False,
                             device_id=self.house_id, available_current=current)
